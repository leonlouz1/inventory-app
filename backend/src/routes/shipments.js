...
router.post("/", async (req, res) => {
  try {
    const { pickupDate, carrier, csNumber, notes, orderIds } = req.body;
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const { pickupDate, carrier, csNumber, status, notes, orderIds } = req.body;

    const existing = await prisma.shipment.findUnique({ where: { id }, include: { orders: true } });
    if (!existing) return res.status(404).json({ error: "Not found" });

    // When marking as PICKED_UP, auto-mark all linked orders as SHIPPED
...
router.post("/:id/orders/:orderId", async (req, res) => {
  try {
    ...
    await prisma.order.update({ where: { id: orderId }, data: { shipmentId: id } });
    const shipment = await prisma.shipment.findUnique({ where: { id }, include: { orders: true } });
    res.json(serialize(shipment));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
